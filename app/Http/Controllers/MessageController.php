...
public function storeStudent(Request $request)
{
    ...
    if (!$student) {
        // Handle case where the student is not authenticated
    ...
    foreach ($admins as $admin) {
        $message = new Message();
        $message->sender_id = $student->formatted_id; // Ensure sender_id is correctly set
        $message->sender_type = Siswa::class;
        $message->receiver_id = $admin->formatted_id; // Ensure receiver_id is correctly set
    ...
    {
        $student = Auth::guard('siswa')->user();
        $messages = Message::where('receiver_id', $student->formatted_id)
            ->where('receiver_type', Siswa::class)
            ->orWhere(function ($query) use ($student) {
                $query->where('sender_id', $student->formatted_id)
                    ->where('sender_type', Siswa::class);
            })
            ->with('sender')
